...
export const MemberDashboard: React.FC<MemberDashboardProps> = ({
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 max-w-7xl mx-auto font-sans">
      ...
      {/* Welcome Banner */}
      <div className="bg-gradient-to-br from-indigo-950 to-indigo-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-indigo-950/20 relative overflow-hidden">
      ...
      {hasNoPoles && (
        <div className="p-5 sm:p-6 rounded-3xl bg-indigo-900 text-white shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center flex-shrink-0 text-white font-bold">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-white text-indigo-900 hover:bg-indigo-50 rounded-2xl text-xs font-extrabold shadow-xs transition-all flex items-center gap-1.5 flex-shrink-0"
          >
            ...
            {myPoles.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {myPoles.map((p: any) => (
                  <div
                    key={p.id}
                    onClick={() => onNavigateToPole?.(p.id)}
                    className="p-3 rounded-2xl border border-slate-100 bg-slate-50/50 flex items-center justify-between cursor-pointer hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors group"
                  >
                    <div className="flex items-center gap-2.5">
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: p.color || '#4f46e5' }} />
                      <div>
                        <p className="text-xs font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{p.name}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white p-5 sm:p-6 rounded-3xl border border-slate-200/90 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onOpenUnavailabilityModal}
              className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 text-xs font-bold rounded-xl flex items-center gap-1"
            >
        ...
        <div className="lg:col-span-5 bg-white p-5 sm:p-6 rounded-3xl border border-slate-200/90 shadow-sm flex flex-col justify-between">
                ...
                {birthdays.map((b: any, i: number) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-2 rounded-xl transition-colors ${
                      b.isToday ? 'bg-indigo-50/70 border border-indigo-200' : 'hover:bg-slate-50'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      {b.avatar ? (
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                          b.isToday ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600'
                        }`}
                      >
          ...
          <button
            ...
            className="w-full mt-4 pt-3 border-t border-slate-100 text-xs font-semibold text-indigo-600 hover:text-indigo-700 flex items-center justify-center gap-1"
          >

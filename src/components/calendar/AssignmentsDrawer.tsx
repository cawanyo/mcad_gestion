...
export const AssignmentsDrawer: React.FC<AssignmentsDrawerProps> = ({
  ...
  const filteredMembers = (poleMembers.length > 0 ? poleMembers : allUsers).filter((m) => {
      ...
      <div className="bg-white w-full max-w-lg h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-3">
          ...
          {filteredMembers.map((m) => {
            ...
            const hasUnavail = m.unavailabilities && m.unavailabilities.length > 0;
            const unavail = hasUnavail ? m.unavailabilities[0] : null;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">
                      {assignment ? assignment.roleTag || 'Affecté' : (m.phone || 'Disponible')}
                    </p>
                    {hasUnavail && !isAssigned && (
                      <p className="text-[10px] text-rose-600 font-medium">
                        ⚠️ Indisponible : {unavail?.reason || 'Période bloquée'}
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  {isAssigned ? (
                    <div className="flex items-center gap-1.5">
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700">
                        ...
                        <span>{isAssignedToThisPole ? 'Affecté' : 'Autre pôle'}</span>
                      </span>
                      <button
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-rose-100 text-rose-700 hover:bg-rose-200 transition-colors"
                    >
